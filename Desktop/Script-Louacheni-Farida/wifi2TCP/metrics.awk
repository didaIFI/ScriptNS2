...
#    pkt_size=124;	# For 97 Bytes ZigBee packets
    pkt_size=1069;	# For 1000 Bytes WiFi packets
...
# Check for a MAC packet
    ...
    if ( type == "AGT" ) 
	{
    	    ...
    	    if ( event == "s" )
		{
        	    ...
        	    # rate
                    if (mac_sub_type != "ack")  # for tcp ack packets
                       {
        	       packets_sent[node]++;
...
#    	printf ("%d - %d\n", sources[q], dests[q]);
    ...
    for (i in nodes) 
    	{
    	    if ( packets_received[i] > 0 ) 
    		{
        	    ...
        	    if ( runtime > 0 ) 
    			{
            		    throughput[i] = packets_received[i]*pkt_size*8/ runtime;
    	    ...
    	    if ( packets_sent[i] > 2 ) 
    		{
        	    ...
        	    runtime = end - start;
        	    if ( runtime > 0 ) 
        	    	rate[i] = (packets_sent[i]*pkt_size*8) / runtime;
    		}
    	}
    ...
    if ( total_runtime > 0 && total_AGTpackets_sent > 0)
        load = ((total_AGTpackets_sent * pkt_size*8)/total_runtime) ; # n=o overhead

	printf("Avg MAC Throughput (bps): %d\nMinimum Delay: %f\nMaximum Delay: %f\nAverage Delay: %f\n", 
           average(throughput),
           min(delay),
           max(delay),
           average(delay));
    ...
    printf("\nData Packets Transmitted: %d\nData Packets Received: %d\nPackets Dropped: %d\nData Packet Delivery Ratio: %f\n", 
	   total_AGTpackets_sent,
	   total_AGTpackets_received,
	   total_AGTpackets_dropped,
	   pkt_delivery_ratio);
...
#    ************** Node Energy Consumption ****************
    ...
    printf("\ninitial_energy: %f\nenergy_used: %f\npercent_energy_used: %f\nmax(csma_analyze): %d\nmin(csma_analyze): %d\nmax(csma_analyze)-min(csma_analyze): %d\n", 
	   initial_energy[0],
	   energy_used[0],
	...
	printf ("\nTypes of dropped packets:\nAPS, LQI, END_, COL, DUP, ERR, RET, STA, BSY, NRTE, LOOP, TTL, TOUT, CBK, IFQ, ARP, OUT, hashes, others\n");  
