...
MainWindow::setIpAddress(char *host)
{
    ipAddressDestination = host;
}
...
void startThreadSendVideo(MainWindow* mainWindows)
{
    ...
    while(1) {
        ...
        n=sendto(mainWindows->sockfdSendVideo,data,data_len,0,(struct sockaddr*)&(mainWindows->servaddrSendVideo),sizeof(mainWindows->servaddrSendVideo));
        printf("video send %d \n",n);
        //Afficher a l'ecran
...
{
    QFuture<void> futureSendVideo = QtConcurrent::run(startThreadSendVideo, this);
    qDebug() << "start thread to send Video" << QThread::currentThread();
}
